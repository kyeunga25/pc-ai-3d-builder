...
- Owner/admin-only member directory with 100-record protected pages, header-only private cursors, D1 invitations, least-privilege role controls, reactivation and two-step suspension. The interface reports loaded counts, keeps existing members after a recoverable page failure and never renders private member IDs.
...
- Separate synthetic CSV and TSV template downloads plus transactional import of at most 50 catalogue records through one shared validated schema. Browser size/type/schema preflight improves feedback without replacing authoritative Worker validation.
- Traditional-Chinese-first bilingual Catalogue status and recovery copy for product writes, two-step archive, private asset-draft creation and CSV／TSV import, including semantic info/success/warning/error announcements and reload guidance after an ambiguous write result.
...
- Four private source-image views (`front`, `back`, `left`, `three-quarter`) plus one GLB, with shared filename/MIME/complete-bytes browser preflight, canonical upload MIME, independent Worker signature/MIME/size validation, safe per-slot metadata, Traditional-Chinese-first bilingual requirements, selectable upload/replace/remove/progress controls and Access-boundary guidance. Valid files with empty or generic browser MIME remain usable when a recognized extension and content agree; conflicts fail before network work. Replacement or removal warns that all approval evidence is reset and every other private file remains private. Removal requires a second activation for the unchanged workspace, asset version, kind and selected view before the Worker receives a bodyless request.
