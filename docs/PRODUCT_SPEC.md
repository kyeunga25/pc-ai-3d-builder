...
- Four private source-image views (`front`, `back`, `left`, `three-quarter`) plus one GLB, with signature, MIME and size validation, safe per-slot metadata, Traditional-Chinese-first bilingual requirements, selectable upload/replace/progress controls and Access-boundary guidance. Replacement warns that all approval evidence is reset, only the selected view or GLB is superseded and every other private file remains private.
