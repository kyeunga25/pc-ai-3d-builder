...
Workers Runtime integration tests apply the real migrations and use Miniflare/workerd D1, R2 and Workflow bindings. Catalogue-import tests verify workspace-scoped SKU uniqueness, replay rejection and full rollback of earlier rows and audit events when a later insert conflicts. Private-asset tests verify owning-workspace reads and hide both reads and replacements from another workspace without mutating D1 or R2 state. Persistent-build tests verify owning-workspace detail and portable export, cross-workspace not-found behavior, warning-only export and fail-closed hard-error or unknown compatibility results against real D1 rows. Generation tests force a transient post-storage validation retry and verify one provider attempt, one reservation, deterministic private output, read-back validation, idempotent request replay and exact-once approval settlement. A terminal validation failure verifies draft cleanup and exact-once release.
