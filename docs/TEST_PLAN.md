...
- 50-record asset-review pages with one look-ahead row, bounded header-only composite cursors, URL/malformed/cross-workspace rejection, stable replay after cursor-asset review, client deduplication, loaded-position copy, abort-safe next-page retry and unsaved-form switching denial;
...
- exhaustive bilingual Builder-viewport coverage for four cameras, three display modes, ten component steps and four stock states; canonical approved-model selection with per-category deduplication and a nine-model cap; parallel private reads; local synthetic isolation; partial success; late cancellation and exact object-URL release; one-canvas normalized review-grid rendering; distinct authorized-private, local-synthetic, private-failure, local-fallback and no-approved-model copy; camera/mode/grid readouts; private-ID absence; the non-compatibility and non-installation evidence boundaries; a wrapping content-height footer; and a visually hidden phone-width figure caption without page-level overflow;
...
Asset-review queue integration coverage additionally seeds 52 current-workspace records plus a foreign record, verifies a 50/2 split, mutates the first page's cursor asset, replays the same continuation without a skip, and rejects a valid foreign-workspace cursor.
