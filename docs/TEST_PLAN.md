...
- fixed-URL header-only private-file targets with viewer and malformed-target rejection before body/D1/R2; exact source-view enumeration, `front` backward-compatible default and model/source-view rejection; generic legacy-path logging; workspace-scoped R2 creation with upload checksum enforcement; selected-view replacement isolation; exact-key rollback cleanup; one bounded transient delete retry; persistent-failure suppression after two attempts; and reads that fail closed on missing or drifted size, content type or SHA-256;
...
- enumerated bilingual four-view source-image and GLB requirements plus selected-view upload, replace and progress labels; rendered Access-boundary and complete evidence-reset guidance; accurate selected-file replacement scope; and narrow-layout wrapping without page-level overflow;
