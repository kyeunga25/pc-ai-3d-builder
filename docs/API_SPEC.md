...
Requires an owner or admin role and returns at most 50 meaningful audit transitions from the resolved workspace, ordered by creation time and an internal tie-breaker. The response maps known actions to bounded public action/category enums and returns only a control-character-free actor display name or a system marker plus the timestamp. It does not return email, target ID, request ID, object location or raw `metadata_json`; an unknown stored action becomes `other` rather than exposing its value. If another page exists, the protected response supplies a bounded `nextCursor` which the browser replays only in `X-RigStage-Workspace-Activity-Cursor`. Query-string cursors, malformed cursors and cursors from another workspace are rejected. Each read is `private, no-store` and appends no audit event.
...
Returns at most 50 draft or in-review assets from the resolved workspace, with their catalogue identity, fixed checklist identifiers, source-rights flag, human-verified dimensions, review version and safe file metadata. The response always contains `items` plus a nullable, bounded `nextCursor`. The browser keeps that cursor in memory and returns it only through `X-RigStage-Asset-Review-Cursor`; URL query parameters are rejected before queue-specific D1 work. A cursor contains the prior page's stable ordering marker, is accepted only after its asset is found in the resolved workspace and is transport state rather than an authorization token. This permits retrying the same continuation after the cursor asset is reviewed without skipping the next records, while a malformed or cross-workspace cursor returns bilingual `VALIDATION_ERROR`. Pages use `private, no-store`.

The file projection always has `front`, `back`, `left` and `three-quarter` source slots plus one model slot; an available slot contains only validated content type and bounded byte size. It never returns private object keys, checksums or provider responses. The client appends pages without duplicate asset IDs, reports loaded rather than unknown total counts, and blocks previous/next switching while the current review form has unsaved changes or a mutation/file/generation operation is pending. A next-page failure preserves the current asset and offers a safe retry. A separately selected asset remains an isolated one-record view and does not expose queue navigation.
...
Requires a bounded asset ID in `X-RigStage-Asset-Id`, `source` or `model` in `X-RigStage-Asset-File-Kind`, and the current version in `X-RigStage-Expected-Version`. A source request selects `front`, `back`, `left` or `three-quarter` through `X-RigStage-Asset-Source-View`; omitting it defaults to canonical `front` for backward compatibility. A malformed source view, or any source-view header on a model request, returns bilingual `VALIDATION_ERROR` before D1, body or R2 work. The route replaces that selected view or model for a non-approved asset; neither the fixed URL nor binary body contains the private asset ID or local filename. The browser uses the same filename/MIME/bytes preflight as source creation and sends only canonical `image/jpeg`, `image/png`, `image/webp` or `model/gltf-binary`. Viewer roles fail before the Worker reads either target, the body, D1 or R2. Missing or malformed targets fail before body or storage work. Source files use the same authoritative image rules as creation. Models must be a complete, self-contained glTF 2.0 GLB with a matching binary header and declared length, at most 25 MiB. Before storage, manual models receive the same buffer／bufferView／accessor bounds, triangle-only primitive checks, node-graph and effective-dimension limits, and texture count／byte budgets as generated drafts. External resource URIs and unbounded renderable structures are rejected.

A successful replacement increments the review version and resets the checklist, source-rights confirmation, dimensions and approval state. Canonical `front` metadata remains on `product_assets`; each additional view is upserted into the workspace-scoped `product_asset_source_files` table in the same conditional D1 batch as the review-version transition and minimal audit event. The new R2 object is stored with its SHA-256 integrity check before that batch. Database triggers recheck that the linked catalogue part is still active at this write boundary. If an archive wins the race, the whole D1 batch rolls back, the new object is removed, the previous selected-view object remains and workspace-safe `ASSET_NOT_FOUND` is returned. Reactivating the part permits a retry with the unchanged asset version. After a committed transition, only the previous object for the selected view or model is removed separately; all other private files remain unchanged.

## `DELETE /api/assets/item/file`

Uses the same fixed URL and bounded asset, file-kind, optional source-view and expected-version headers as replacement, but accepts no request body. A source request removes only `front`, `back`, `left` or `three-quarter`; a model request must omit the source-view header. Viewer denial happens before target, D1 or R2 work. Missing or malformed targets remain workspace-safe, malformed or inapplicable views and versions return bilingual `VALIDATION_ERROR`, a missing selected file returns `ASSET_FILE_NOT_FOUND`, stale versions return `ASSET_VERSION_CONFLICT`, and approved assets return `ASSET_LOCKED`. None of those rejected paths deletes an object.

The selected metadata is nulled or deleted together with the asset review-version increment, complete approval-evidence reset, minimal `asset.file.{kind}.remove` audit event and any exact-once release of a reserved generated draft in one guarded D1 batch. The update requires the recorded object key, current version, unapproved state, resolved workspace and still-active catalogue relationship. An archive or concurrent file change therefore rolls back the complete transition without touching R2. After commit, the Worker deletes only the exact object key that D1 previously selected and retries one transient failure. A second failure is best-effort and may leave a private object that is no longer reachable through current metadata; it does not reverse the committed D1 state or broaden deletion. The response is the same safe asset projection and never contains an object key, checksum or removed target identifier. `X-RigStage-Generation-Credit-Released` is the required `true`／`false` outcome of the authoritative D1 accounting check so the client never infers a credit transition from stale job state; a successful response without that header fails closed as ambiguous and requires a reload.

## `GET /api/assets/item/file`

Requires the same bounded `X-RigStage-Asset-Id` and `X-RigStage-Asset-File-Kind` target headers. Source reads use the same optional `X-RigStage-Asset-Source-View` selection and `front` default; model reads reject that header. A malformed or inapplicable source-view header returns bilingual `VALIDATION_ERROR` before D1 or R2. The route streams the selected private source or model only after Access verification and active workspace membership resolution. The private asset ID is absent from the fixed URL. Before streaming, the R2 object's byte size, content type and SHA-256 must still match the validated D1 metadata. New objects use R2's stored SHA-256 metadata for the streaming path; an older object without that metadata receives a bounded read-back, file validation and digest comparison before any bytes are returned. A missing, malformed or mismatched asset／file-kind target, metadata or object returns bilingual `ASSET_FILE_NOT_FOUND` without writing state; an R2 or cryptographic operational failure remains an internal failure and can be retried. A successful source response is `private, no-store` with generic `source.ext` or `source-{view}.ext` naming; a model remains `model.glb`. The browser creates temporary object URLs only while the matching workspace/asset request signal remains active and revokes them idempotently on abort or cleanup; a cancelled late response does not create a URL. Permanent object URLs and keys are never returned. Legacy dynamic asset-file paths are not routed.
...
Targets one asset through `X-RigStage-Asset-Id` and creates a zero-monetary-cost simulation job only when the runtime capability is explicitly set to simulation. The private asset ID is absent from the fixed URL and JSON body. The route rejects non-owner/admin roles before reading the target or body; for an authorized role, a missing or malformed target returns bilingual `ASSET_NOT_FOUND` before capability, idempotency, body, D1, R2 or Workflow work. A valid `Idempotency-Key`, an unapproved asset, its current `expectedVersion`, a stored canonical `front` source image, a previously saved source-rights confirmation and at least one available non-monetary generation credit are required. Before reserving credit, the route requires the canonical source R2 object to match the D1-recorded byte size, content type and SHA-256; objects without stored R2 SHA-256 metadata use a bounded read-back. Additional source views are neither read nor described to the provider boundary. Missing or drifted canonical storage returns bilingual `GENERATION_SOURCE_REQUIRED` without creating a job, credit event, audit event or Workflow; an R2 or cryptographic operational failure remains an internal failure and can be retried. Legacy dynamic generation-job paths are not routed.
...
Successful creation returns `202`. The Workflow rechecks the active catalogue state at claim and the same canonical private source size, content type and SHA-256 before beginning a provider attempt, closing races after route preflight. A catalogue archive before claim records `GENERATION_INPUT_STALE`, releases once and makes no provider attempt. The final staging update rechecks active catalogue state at the D1 write boundary; if a later archive wins, the private draft is removed and the reservation is released once instead of creating an invisible review draft. If the canonical source disappears or drifts, stable failure code `GENERATION_INPUT_MISSING` releases the reservation once without a provider attempt; replaying the original idempotency key returns that terminal job. The Workflow output remains a draft and resets all prior checklist and dimension evidence before its state becomes `awaiting_review`. Approval settles the reservation. Rejection, terminal failure, Workflow-start failure or generated-draft replacement releases it once. Tracked production configuration returns `GENERATION_DISABLED` before any database write or external activity. An absent/empty credit account returns `GENERATION_CREDITS_REQUIRED` without creating a job or provider attempt.

## `DELETE /api/assets/item/generation-jobs`

Cancels one exact job only while it remains `queued`. The request is bodyless and carries bounded `X-RigStage-Asset-Id` and `X-RigStage-Generation-Job-Id` headers; neither identifier enters the fixed URL. Owner/admin role is checked before either target or body state, and malformed, missing, cross-workspace or asset/job-mismatched targets return the same bilingual `GENERATION_JOB_NOT_FOUND` before mutation. A supplied body returns `UNEXPECTED_REQUEST_BODY` without consuming or reflecting it.

One guarded D1 batch changes `queued` to `cancelled`, records a minimal job event and audit action, and releases the one reserved non-monetary credit with reason `user_cancelled`. A replay of the same already-cancelled job returns its terminal public representation without another release or event. If Workflow changes the job to `running` first, the conditional update changes nothing, `GENERATION_CANCEL_TOO_LATE` is returned, and the reservation and processing remain untouched. A Workflow instance that claims after cancellation returns a terminal `cancelled` result before source reads or provider attempts. The response exposes no Workflow ID, requester, object key, checksum or provider reference.
