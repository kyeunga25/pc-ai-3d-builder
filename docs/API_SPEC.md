...
Returns at most 50 draft or in-review assets from the resolved workspace, with their catalogue identity, fixed checklist identifiers, source-rights flag, human-verified dimensions, review version and safe file metadata. The file projection always has `front`, `back`, `left` and `three-quarter` source slots plus one model slot; an available slot contains only validated content type and bounded byte size. It never returns private object keys, checksums or provider responses.
