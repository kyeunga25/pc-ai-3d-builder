...
The Catalogue list shows page guidance, actions, search, category and verification filters, columns, category, stock, asset state, quality, quantity and row actions in Traditional Chinese and English. Separate synthetic CSV and TSV template controls identify their downloaded format; one bilingual import control accepts only CSV／TSV, reports preflight and authoritative validation through the existing semantic status channel, and stays disabled for viewers. Unknown stock is not presented as available; status uses text and icons in addition to colour. At compact widths, page actions and live status wrap, filters can move to separate rows and the simplified product row retains a bilingual accessible View action without page-level horizontal overflow.
...
The workspace member page is visible only to owner/admin roles. It presents invitation guidance, exact Access-policy separation, identity/membership state, role controls and two-step suspension in Traditional Chinese and English. It reports how many protected members are loaded and shows one bilingual continuation control only when another 100-record page exists; continuation disables conflicting actions, preserves the current list on failure and can be retried. Emails remain inside the protected view, member IDs and pagination cursors are never visible, self-management and unauthorized targets have explicit disabled explanations, and the desktop grid becomes one column without horizontal overflow at compact widths.

The workspace activity page is visible only to owner/admin roles. It presents meaningful action, category, safe actor display name and local time in Traditional Chinese and English, with client-side category filtering and an explicit load-older action. It never renders an event, target or request ID, email, object location, raw metadata or pagination cursor. Cards and filters wrap into a single readable column at compact widths, while an absent actor is labelled as a system workflow rather than guessed identity.
...
The Builder viewport shows camera presets, display modes, fit-view control, scene readouts, selected component, stock state and model provenance in Traditional Chinese and English. A category view shows at most one approved model. Summary view uses one canvas for up to nine approved selected-component models in canonical category order; independent loads may partially succeed, and the visible count reflects only decoded models. Its separated review grid normalizes display size and must state that it is not installation position, physical scale or compatibility evidence. Authorized private loading, local synthetic preparation, partial or complete load failure and static fallback remain visibly distinct. The footer uses content-driven height and wrapping for preview provenance, the visual-evidence limitation and scene units; at 520 px or narrower the long figure caption becomes visually hidden while equivalent footer copy and the complete bilingual accessible canvas name remain available. Compact widths may hide secondary controls but retain bilingual accessible names and must not create page-level overflow.
...
Private source-image and GLB controls show Traditional Chinese and English requirements, supported filename extensions, filename/MIME/content preflight, the currently selected source view, upload/replace/remove/progress actions and the Access boundary without exposing an asset identifier or sending the local filename. Empty or generic browser MIME can be normalized only after a recognized extension and complete content validate; conflicts produce a bilingual pre-network error. Replacing or removing any file visibly warns that the checklist, source-rights confirmation and verified dimensions reset; only the selected view or GLB changes, while every other private file remains private. A removal control appears only when that exact slot exists. Its first activation changes to a warning-coloured explicit confirmation and sends no request; only a second activation for the unchanged workspace, asset version, kind and view submits. Switching views or beginning another upload, review or generation action disarms it. At narrow widths, labels and controls wrap into a vertical layout without horizontal overflow.

The source filmstrip is an operable single-selection control for `front`, `back`, `left` and `three-quarter`. Exactly one thumbnail exposes `aria-pressed=true` and controls the source upload panel; each thumbnail labels its authorized or missing private-image state, real preview or actionable missing state, and the filmstrip shows confirmed or missing usage-rights evidence in both languages. Rights state uses an icon and text in addition to success or warning colour. At phone width, bilingual view badges stay bounded inside each thumbnail while the filmstrip remains an internally scrollable row.
...
The generation inspector includes an internally scrollable history of at most 20 exact current-asset jobs in server order. Each row shows only a bilingual latest marker where applicable, lifecycle status, entitlement status and Hong Kong update time. Invalid time is labelled rather than guessed; another asset, job ID, failure code, provider and private object data never render. The list wraps without creating page-level horizontal overflow at 390 px.
