...
Private source-image and GLB controls show Traditional Chinese and English requirements, the currently selected source view, upload/replace/progress actions and the Access boundary without exposing an asset identifier. Replacing any file visibly warns that the checklist, source-rights confirmation and verified dimensions reset; only the selected view or GLB is superseded, while every other private file remains private. At narrow widths, labels and controls wrap into a vertical layout without horizontal overflow.
