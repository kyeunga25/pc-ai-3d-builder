...
- The private source image must exist with R2 byte-size, content-type and SHA-256 metadata matching D1, or pass a bounded read-back checksum fallback for an older object, and its usage-rights check must already be saved at the current review version.
...
- `Idempotency-Key` prevents duplicate job creation. A replay must retain both the original asset and requested review version; changing either input returns a bilingual conflict before Workflow or credit activity. A new request is blocked while the same asset has an active job or an `awaiting_review` job with a reserved entitlement.
