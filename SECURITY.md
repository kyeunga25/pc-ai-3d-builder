...
- The bilingual Builder command bar derives workspace, build, navigation, create, two-step archive and inspector labels from one typed presentation contract. Viewer rendering withholds create and archive controls, while server-side role checks remain authoritative. Compact icon controls keep accessible bilingual names, and visible labels never substitute a private build ID for its workspace-scoped display name.
