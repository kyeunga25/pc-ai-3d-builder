...
- The landing header, hero, three workflow narratives, three merchant use cases, all synthetic screenshot descriptions, trust statements and sign-in boundary are typed bilingual public copy. Their rendered links are limited to same-origin entry/demo roots, in-page sections and tracked local synthetic images; they contain no workspace or protected-record target. Public workflow and trust copy describe the boundary but never substitute for server authorization.
- The public login UI reduces any requested destination to one of four fixed workspace route roots before rendering a same-origin Access navigation. Its typed bilingual copy exposes only generic destination names and never renders deep-route segments, query strings, fragments or dynamic record identifiers. This presentation boundary does not grant access; Access verification and active server-side membership remain authoritative.
...
- The Catalogue list is a typed bilingual projection of already workspace-scoped records. Its exhaustive category, stock, asset-state and quality labels do not render part or asset identifiers, do not reinterpret unknown stock as available and do not grant write access; viewer import and creation controls remain disabled while Worker role checks stay authoritative.
- The Catalogue editor is another typed bilingual projection of the selected workspace-scoped record. It shares the list's exhaustive category and stock maps, renders SKU and optimistic version instead of private part or asset IDs, and leaves its complete field set disabled for viewers while omitting save, archive and source-draft controls. These presentation constraints do not replace the Worker's role, workspace, target or version checks.
...
- Private image and GLB uploads enforce role, workspace, MIME and bounded-size checks. Source images must be static, no larger than 8,192 px per edge or 24 MP, and contain structurally complete PNG chunks with CRCs, JPEG marker sequences or WebP RIFF chunks; APNG and animated WebP are rejected before storage. Manual and generated GLBs share strict buffer, accessor, triangle, node-graph, dimension and texture budgets and cannot reference external resources. These structural checks are not image-authenticity or visual-content moderation.
...
- Private owner onboarding and deployment read values only from private environments, require a mode-`0600` ignored deployment config with alternate public Worker URLs disabled, and suppress Wrangler stdout/stderr that could contain identity, resource mappings, routes or version coordinates. Standard logs receive only stable generic status.
