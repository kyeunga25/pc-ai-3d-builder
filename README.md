...
- 固定 URL 的生成工作 API 以受保護 asset 標頭指定目標，並受 workspace、角色、素材版本、已儲存使用權、可用 generation credit 及 `Idempotency-Key` 限制；asset ID 與 key 均不進 URL 或 body，瀏覽器只在結果不明時以記憶體保留同一素材版本的 key 作安全重試，同一素材只可有一項進行中或等待人工決定的保留工作。
