...
- Builder 頂部命令列以繁中優先、英文輔助顯示 workspace、目前組裝、組裝名稱、返回 Dashboard、建立、兩步封存及檢查器入口。固定文案由完整型別映射供應；viewer 不會取得新增或封存控制，390 px 圖示入口仍保留雙語無障礙名稱，畫面標籤不會把私人 build ID 當作可見文案。
