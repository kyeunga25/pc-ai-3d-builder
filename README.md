...
- 受 workspace、角色、素材版本、已儲存使用權、可用 generation credit 及 `Idempotency-Key` 限制的生成工作 API；瀏覽器只在結果不明時以記憶體保留同一素材版本的 key 作安全重試，不寫入 URL 或持久儲存，同一素材只可有一項進行中或等待人工決定的保留工作。
