...
function stockLabel(status: (typeof catalogParts)[number]["stockStatus"]) {
  switch (status) {
    case "in_stock":
...
function assetLabel(status: (typeof catalogParts)[number]["assetStatus"]) {
  switch (status) {
    case "approved":
...
function assetQualityLabel(
  quality: (typeof catalogParts)[number]["assetQuality"],
) {
  switch (quality) {
    case "unreviewed":
      return "尚未評級";
...
export function CataloguePage() {
  ...
  const [notice, setNotice] = useState("24 件產品 · 模擬目錄");
  ...
  return (
    <div className="page catalogue-page">
      <header className="page-header">
        <div>
          <span className="eyebrow">已核實商戶庫存</span>
          <h1>產品目錄</h1>
        ...
        <div className="page-header__actions">
          <button
            ...
            type="button"
            onClick={() => setNotice("已開啟 CSV 匯入預覽 · 模擬狀態")}
          >
          ...
          <button
            ...
            type="button"
            onClick={() => setNotice("新增產品表格已準備 · 模擬狀態")}
          >
      ...
      <section className="catalogue-toolbar" aria-label="產品目錄篩選器">
        ...
        <button className="button catalogue-filter-button" type="button">
